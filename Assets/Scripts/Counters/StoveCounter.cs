...
public class StoveCounter : BaseCounter, IHasProgress
{
    ...
    private void Update()
    {
        if (HasKitchenObject() && (fryingRecipeSO != null || burningRecipeSO != null))
        {
            switch (state)
            {
                case State.Idle:
    ...
    public override void Interact(Player player)
    {
        if (!HasKitchenObject())
        {
            if (player.HasKitchenObject())
            {
                ...
                if (HasFryingRecipeWithInput(GetKitchenObject().GetKitchenObjectSO()))
                {
                ...
                else if (HasBurningRecipeWithInput(GetKitchenObject().GetKitchenObjectSO()))
                {
                    burningRecipeSO = GetBurningRecipeSOWithInput(GetKitchenObject().GetKitchenObjectSO());
